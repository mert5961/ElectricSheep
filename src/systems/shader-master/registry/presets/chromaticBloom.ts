...
void main() {
  ...
  float motionDrive = 0.14 + (1.0 - stillness) * 0.94 + densityFeel * 0.14;
  float t = u_time * u_speed * motionDrive;
  float radius = length(centered);
  float angle = atan(centered.y, centered.x);

  float abAmount = u_aberration * (1.0 + snare * 3.0 + tension * 2.2 + fragmentation * 0.85);
  ...
  float hihatGrain = noise(centered * (18.0 + treble * 28.0 + densityFeel * 16.0) + vec2(t * 7.0, -t * (5.5 + fragmentation * 2.5)));
  hihatGrain = pow(saturate(hihatGrain - (0.62 - fragmentation * 0.1)), 2.5) * hihat;
  ...
  float orbit2 = bloomShape(
    centered - vec2(cos(t * 0.8 - 1.5), sin(t * 1.1 + 0.5)) * (0.4 + flux * 0.15 + densityFeel * 0.14),
    0.12 + flux * 0.06 + densityFeel * 0.08
  ) * flux * (0.3 + densityFeel * 0.12);
  float aura = bloomShape(centered, u_bloomSize * (1.16 + glowFeel * 0.55 + densityFeel * 0.3));

  float vignette = 1.0 - smoothstep(0.6, 2.2, radius);

  vec3 warmTint = mix(vec3(0.9, 0.96, 1.08), vec3(1.18, 0.92, 0.76), warmth * 0.86 + rumble * 0.12);
  ...
  vec3 color = vec3(redCh, greenCh, blueCh) * (0.42 + glowFeel * 0.62 + warmth * 0.08);
  ...
  color += mix(u_colorA, u_colorB, 0.5) * aura * glowFeel * (0.18 + densityFeel * 0.16);
  color += vec3(snareAb * 0.15, 0.0, snareAb * 0.1);
  color *= warmTint;
  color *= vignette;
  color *= 0.65 + u_intensity * 0.5 + rumble * 0.1 + fragmentation * 0.1 + glowFeel * 0.06;
