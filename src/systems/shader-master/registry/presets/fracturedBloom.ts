...
void main() {
  ...
  float tension = saturate(u_feelTension + (rumble * 0.3));
  float warmth = saturate(u_feelWarmth);
  float fragmentation = saturate(u_feelFragmentation + (flux * 0.35) + (treble * 0.2));
  ...
  float kickFlash = exp(-radius * 5.0) * kick * 0.6;
  float snareShock = exp(-abs(radius - (0.4 + snare * 0.15)) * 16.0) * snare * 0.7;
  float noiseBand = fbm(
    warped * (3.0 + (treble * 2.4) + (densityFeel * 1.1) + (flux * 1.5))
    + vec2(
