...
void main() {
  ...
  float motionDrive = (0.14 + ((1.0 - stillness) * 1.18)) * (0.78 + (energy * 0.4) + (glow * 0.08));
  ...
  float warpAmount = ((u_scanWarp * 0.1) + (mid * 0.14) + (tension * 0.16) + (bass * 0.06))
    * (0.22 + ((1.0 - stillness) * 1.14));
  ...
  ) * tension * (0.02 + (fragmentation * 0.08));

  float density = u_lineDensity + (densityFeel * 14.0) + (mid * 7.0) + (midBreath * 1.8) + (tension * 6.0) + (bass * 4.5);
  float lineSharpness = mix(0.22, 0.05, saturate((tension * 0.85) + (fragmentation * 0.5)));
  ...
  float segmentBreak = mix(1.0, mix(0.32, 1.28, fragmentCells), fragmentation * 0.84);

  vec3 thermalShift = mix(vec3(0.9, 0.98, 1.08), vec3(1.16, 0.92, 0.72), warmth * 0.72);
  vec3 coolBase = mix(u_colorB * vec3(0.84, 0.96, 1.08), u_colorA * 0.18, frame);
  vec3 warmBase = mix(u_colorB * vec3(1.16, 0.86, 0.62), u_colorA * vec3(1.12, 0.9, 0.72) * 0.3, frame);
  vec3 base = mix(coolBase, warmBase, warmth * 0.96);
  vec3 lineColor = mix(
    mix(u_colorA, u_colorA * vec3(1.18, 0.88, 0.68), warmth * 0.92),
    vec3(1.0),
    saturate((pulse * 0.28) + (glow * 0.46) + (treble * 0.52) + (tension * 0.3) + (energy * 0.18))
  );
  ...
  color *= 0.74 + (u_intensity * 0.32) + (tension * 0.18) + (energy * 0.08);
  color += frame * (0.04 + (bass * 0.05)) * mix(u_colorA, lineColor, 0.5 + (warmth * 0.35));
  color += lineColor * frame * tension * 0.08;
  color += vec3(1.0, 0.95, 0.86) * glow * pulse * 0.16;
  color *= thermalShift;
